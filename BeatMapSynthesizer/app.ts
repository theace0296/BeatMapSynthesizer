 ...
 */
ipcMain.on('__generateBeatMap__', function (event, dir: string, difficulty: string, model: string, k: number = 5, version: number = 2) {
    // Integrate this IPC for generating a beat map
    let options: Options = {
        mode: 'text',
        pythonPath: path.join(app.getAppPath().toString(), "build/python/python.exe"),
        pythonOptions: ['-u']
    };

    PythonShell.runString(`import subprocess;import sys;import os;subprocess.check_call([sys.executable, '-m', 'pip', 'install', '--upgrade', 'pip']);subprocess.check_call([sys.executable, '-m', 'pip', 'install', '-r', '${path.join(app.getAppPath().toString(), '/build/scripts/py_requirements.txt').normalize().replace(/\\/gi, "/")}'])`, options, function () { /* Callback not used */ })
        ...
        .on('close', function () {
            ...
            mm.parseFile(dir).then(metadata => {
                ...
                PythonShell.run(path.join(app.getAppPath().toString(), '/build/scripts/beatmapsynth.py'), options, function () { /* Callback not used */ })
                    .on('message', function (message: string) {
                        mainWindow.webContents.send('task-log-append-message', message);
                    })
                    .on('stderr', function (err: PythonShellError) {
