...
const __coreCount = (() => {
    ...
    else if ((os_1.totalmem() / 2147483648) <= workingCores) {
        return Math.floor(os_1.totalmem() / 2147483648);
    }
    return workingCores;
})();
/**
...
class worker {
    ...
    async initFiles() {
        return new Promise(resolve => {
            ...
            if (process.platform == 'win32') {
                ...
                child_process_1.exec('python3 --version', (error, stdout) => {
                    if (error) {
                        _log(`${error}`);
                    }
                    else {
    ...
    async generateBeatMaps(dir, args) {
        ...
        if (beatMapExists) {
            ...
            if (!util_1.isNullOrUndefined(metadata.common.picture)) {
            ...
            if (!util_1.isNullOrUndefined(embeddedart)) {
        ...
        let song_index = songs_json.length;
 ...
 */
function _appendMessageTaskLog(message, group = "MAIN") {
